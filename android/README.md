...
- Live TMDB/AniList-backed browse, iOS-style TMDB search, detail, and airing schedule flows, including iOS-parity TMDB movie rows for now playing, upcoming, and top-rated movies
...
- Search now matches the iOS TMDB-first tab: no source picker in the main Search screen, recent queries stored locally, multiple TMDB pages fetched, and the backed TMDB horror filter applied to matches
...
- Connected/manual AniList tracker accounts can import the user's AniList anime library through chunked MediaListCollection fetches into Android Library, including resume entries when AniList episode progress can be converted into a percentage
- Connected/manual AniList tracker accounts can also import the user's AniList manga library through chunked MediaListCollection fetches into Android Manga/Novel storage, including chapter progress entries and novel-format items
...
TMDB-powered Home rows, Search results, and episode stills/descriptions require `TMDB_API_KEY`. Android reads it from `android/local.properties`, Gradle properties or environment variables, and falls back to the root `Build.xcconfig` / `Build.local.xcconfig` so local iOS and Android builds can share the same key.
